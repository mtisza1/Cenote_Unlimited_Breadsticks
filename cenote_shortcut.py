...
parser = argparse.ArgumentParser(description='Cenote_Shortcut is a pipeline for virus discovery and cursory annotation of viral contigs and genomes. Visit https://github.com/mtisza1/Cenote_Shortcut for further description')

required_args = parser.add_argument_group(' REQUIRED ARGUMENTS for Cenote_Shortcut ')
...
required_args.add_argument("--mem", dest="MEM", type=int, required=True, help='example: 56 --	Gigabytes of memory available for Cenote_Shortcut. Typically, 16 to 32 should be used. Lower memory will work in theory, but could extend the length of the run ')
required_args.add_argument("--cpu", dest="CPU", type=int, required=True, help='Example: 32 --	Number of CPUs available for Cenote_Shortcut. Typically, 32 CPUs should be used. For large datasets, increased performance can be seen up to 120 CPUs. Fewer than 16 CPUs will work in theory, but could extend the length of the run ')




optional_args = parser.add_argument_group(' OPTIONAL ARGUMENTS for Cenote_Shortcut. Most of which are important to consider!!! ')
